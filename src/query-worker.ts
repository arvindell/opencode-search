...
type Session = { id: string; title: string; updated: number };
type Request =
  | { type: "search"; file: string; query: string; sessions: Session[] }
  | {
      type: "details";
      file: string;
      sourceFile: string;
...
function titleScore(title: string, query: string, terms: string[]) {
  const value = normalize(title);
  if (value.includes(query)) return 120;
  const words = value.split(" ");
  const scores = terms.map((term) => {
    if (words.some((word) => word.startsWith(term))) return 85;
...
function search(input: Extract<Request, { type: "search" }>) {
  ...
  try {
    ...
    const getHits = (expression: string) =>
      db
        .query(
          `SELECT indexed_sessions.id AS id
           FROM session_text JOIN indexed_sessions ON indexed_sessions.rowid = session_text.rowid
           WHERE session_text MATCH ? ORDER BY bm25(session_text)`,
        )
        .all(expression) as { id: string }[];
    const termQuery = (word: string) => `"${word.replaceAll('"', '""')}"*`;
    ...
    for (const hit of exact) if (!found.has(hit.id)) found.set(hit.id, 80);
    ...
    if (exact.length < 20) {
      const alternatives = terms.map((term) => {
        ...
        const words = db
          .query(
            "SELECT term FROM session_words WHERE length(term) BETWEEN ? AND ?",
          )
          .all(term.length - max, term.length + max) as { term: string }[];
        return [
          term,
          ...words
            .map((row) => ({
            ...
            .filter((item) => item.score > 0 && item.score <= max)
            .sort((a, b) => a.score - b.score)
            .slice(0, 5)
            .map((item) => item.word),
        ];
      });
      for (const hit of getHits(
        alternatives
          .map((group) => `(${group.map(termQuery).join(" OR ")})`)
          .join(" AND "),
      ))
        if (!found.has(hit.id)) found.set(hit.id, 50);
    }

    return input.sessions
      .flatMap((session) => {
        const score = Math.max(
          titleScore(session.title, normalized, terms),
          found.get(session.id) ?? 0,
        );
        return score
          ? [{ id: session.id, updated: session.updated, score }]
          : [];
      })
...
function details(input: Extract<Request, { type: "details" }>): SessionDetails {
  const terms = normalize(input.query).split(" ").filter(Boolean);
  const timeline: DetailLine[] = [];
  const matches: DetailLine[] = [];
  let seen = 0;
  const excerpt = (text: string, at = 0, length = 190) => {
  ...
  const collect = (row: {
    ...
    ) as { text?: string; content?: { type: string; text?: string }[] };
  ...
  if (input.messages) {
    for (const row of input.messages) collect(row);
  } else {
    const db = new Database(input.sourceFile, { readonly: true });
    try {
      const session = db
        .query("SELECT time_updated FROM session_v2 WHERE id = ?")
        .get(input.sessionID) as { time_updated: number } | null;
      ...
      for (const row of db.query(sql).iterate(input.sessionID) as Iterable<{
        type: string;
        time_created: number;
        data: string;
      }>)
  ...
  const shownTimeline =
    timeline.length <= 80 ? timeline : [timeline[0]!, ...timeline.slice(-79)];
  const priority = (line: DetailLine) =>
    line.role === "You" ? 0 : line.role === "Assistant" ? 1 : 2;
  return {
    timeline: shownTimeline,
    userCount,
    assistantCount,
    matches: matches
      .sort((a, b) => priority(a) - priority(b) || b.time - a.time)
      .slice(0, 30),
    matchCount,
  };
}
...
self.onmessage = (event: MessageEvent<Request>) => {
  try {
    ...
    else self.postMessage({ details: details(event.data) });
  } catch (error) {
    self.postMessage({
